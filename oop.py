...
class GAME:
    ...
    def run(self):
        ...
        while running:
            ...
            # self.block.block_rect.x -= self.block.block_speed
            ...
            if self.coin.coin_rect.x < 0:
                self.player_lives-=1
                self.sound.play_miss_sound()
                self.coin.coin_rect.x = WINDOW_WIDTH + self.coin.buffer_distance
                self.coin.coin_rect.y = random.randint(70,self.WINDOW_HEIGHT-32)

            else:
                self.coin.coin_rect.x-= self.coin.coin_speed
            self.surface.fill((0,0,0))

            if self.block.block_rect.x < 0:
                # self.block.block_rect.x = WINDOW_WIDTH + self.block.buffer_distance
                self.block.block_rect.x += self.WINDOW_WIDTH + self.block.buffer_distance
                self.block.block_rect.y = random.randint(70, self.WINDOW_HEIGHT - 40)
                # block_y = random.randint(70, self.WINDOW_HEIGHT - 40)
                # self.block.block_speed += self.block.block_acceleration
                # if self.coin.coin_rect.y == block_y:
                #     Checker(block_y,WINDOW_HEIGHT).check()
                #     self.block.block_rect.x = WINDOW_WIDTH + self.block.buffer_distance
                #     self.block.block_rect.y = block_y
            # self.block.block_rect.y = random.randint(70,self.WINDOW_HEIGHT-40)
            else:
                self.block.block_rect.x -= self.block.block_speed

            #check collision
            if self.dragon.dragon_right_rect.colliderect(self.coin.coin_rect):
                    self.player_score+=1
                    self.sound.play_coin_sound()
                    self.coin.coin_speed += self.coin.coin_acceleration
                    self.coin.coin_rect.x = self.WINDOW_WIDTH + self.coin.buffer_distance
                    self.coin.coin_rect.y = random.randint(70, self.WINDOW_HEIGHT - 32)
            # to move the block

            if self.block.block_rect.colliderect(self.dragon.dragon_right_rect):
                self.sound.play_miss_sound()
                self.player_lives -=1
                self.block.block_speed += self.block.block_acceleration
                self.block.block_rect.x = self.WINDOW_WIDTH + self.block.buffer_distance
            ...
            #     self.block.block_speed += self.block.block_acceleration
            #     self.block.block_rect.x = self.WINDOW_WIDTH + self.block.buffer_distance
            #     self.block.block_rect.y = random.randint(70, self.coin_rect.y - 10, self.WINDOW_HEIGHT - 40)

            # if self.coin.coin_rect.colliderect(self.block.block_rect):
            #     self.coin.coin_rect.y -= self.coin.coin_speed
...
# class Checker:
#     def __init__(self,block_y,WINDOW_HEIGHT):
#         self.block_y = block_y
#         self.window_height = WINDOW_HEIGHT
#     def check(self):
#         rangee = self.window_height//2
#         if self.block_y > rangee:
#             self.block_y -= 20
#             return self.block_y
#         else:
#             self.block_y+=20
...
# class Apple:
#     def __init__(self,dragon,coin,player_lives):
...
#             # self.apple_image_rect -= self.apple_speed
    ...
    def apple_movement(self,updated_lives):
        ...
        if lives == 1:
            ...
            if self.apple_image_rect.colliderect(self.dragon):
                self.apple_lives+=1
                print(f"this is the value of live {self.apple_lives}")
                self.apple_image_rect.x = WINDOW_WIDTH + 50
                self.apple_image_rect.y = random.randint(70, WINDOW_HEIGHT - 40)
                lives+=1
